...
export default function QuizComponent({ countries }) {
  ...
  const handleZakoncz = () => {
    ...
    router.push(`/mainpage/tekstowy/${countries[0]?.difficulty}`);
  };
  const handleHint = () => {
    ...
    <div className="quiz-container flex flex-col items-center justify-top pt-20 min-h-screen px-4 sm:px-6 lg:px-8">
      ...
      {currentQuestion ? (
        <div className="question bg-white shadow-lg rounded-lg p-8 max-w-3xl w-full">
          <h2 className="text-2xl font-bold text-gray-800 text-center mb-6">Odgadnij Kraj</h2>
          <div className="flex justify-center mb-6 text-gray-800 font-semibold text-lg text-center bg-gray-100 p-4 rounded-lg shadow-sm">{currentQuestion.pytanie}</div>
          {hintVisible && <p className="text-green-600 font-semibold mb-4">Kraj zaczyna się na literkę: {currentQuestion.correctAnswer[0][0]}</p>}
          <input
            type="text"
            value={userAnswer}
